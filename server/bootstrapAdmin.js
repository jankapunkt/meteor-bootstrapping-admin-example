...
function runBootstrap () {
  ...
  //
  // Note:
  // To safely check for Strings, we need to check for their length, too.
  ...
  const { firstName, lastName, username, email } = adminSettings
  ...
  if (!userId || !adminId) {
    rollback({ users: userId, admins: adminId })
    throw new Error(`Expected userId (${userId}) and adminId (${adminId})`)
  }
  ...
  // set roles here, if you'd like to use
  // roles/groups, like via alanning:roles
  ...
  try {
    // Implements #5: The Admin person is required to set a password using a password-reset link.
